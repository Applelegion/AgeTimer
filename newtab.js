...
function display_birthday() {
    chrome.storage.local.get("birthday", function (result) {
        ...
        const last_bday = new Date(result.birthday.month.toString() +
            "-" + result.birthday.day.toString() + "-" + current_time.getFullYear().toString() +
            " " + result.birthday.hour.toString() + ":" + result.birthday.minute.toString() + ":00");

        if (current_time - last_bday < 0) {
            last_bday.setFullYear(last_bday.getFullYear() - 1)
        }
        const years_of_age = last_bday.getFullYear() - result.birthday.year;
...
chrome.storage.local.get(["first_use"], function (result) {
    if (result.first_use === false) {
        ...
        const year_list = [];
        for (var i = current_time.getFullYear(); i >= current_time.getFullYear() - 120; i--) {
            year_list.push(i)
        }
        ...
        month_select.addEventListener("change", function () {
            ...
            if (leap_year(current_time.getFullYear(), 0)) {
                for (var i = 1; i <= day_month_map_leap[month]; i++) {
                    day_list.push(i)
                }
            } else {
                for (var i = 1; i <= day_month_map[month]; i++) {
                    day_list.push(i)
                }
            }
            make_options(day_select, day_list)
